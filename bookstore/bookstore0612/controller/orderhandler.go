...
import (
	"go_code/bookstore/bookstore0612/dao"
	"go_code/bookstore/bookstore0612/model"
	"go_code/bookstore/bookstore0612/utils"
	"html/template"
	"net/http"
	"time"
)
...
func Checkout(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range cartItems {
		...
		//更新当前购物项中图书的库存和销量
		book := v.Book
		book.Sales = book.Sales + int(v.Count) //销量
		book.Stock = book.Stock - int(v.Count) //库存
		//更新图书的信息
		dao.UpdateBook(book)
	}
	//清空购物车
